...
{
	global  $context, $smcFunc, $ultimateportalSettings;
	
	$position = !empty($position) ? (string)$position : '';
	...
	if(!empty($ultimateportalSettings['up_reduce_site_overload']))
	{
		if((cache_get_data('up_bk', 1800)) === NULL)
		{
			$result = $smcFunc['db_query']('',"
							...
							WHERE active='checked' ".(!empty($position) ? "and position = {string:position}" : "")."
...
{
	global $settings, $user_info, $options, $ultimateportalSettings;
...
{
	global $context, $options, $ultimateportalSettings, $user_info;
			echo '	
